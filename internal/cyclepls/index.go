...
// Options sets CyclePLS client options
type Options struct {
	URL             string            `json:"url"`
...
type fullRequest struct {
	req     *http.Request
	client  http.Client
	options CyclePLSRequest
}

// Response contains Cyclepls response data
...
// CyclePLS creates full request and response
type CyclePLS struct {
...
// ready Request
func processRequest(request CyclePLSRequest) (result fullRequest) {
...
func (client CyclePLS) Queue(URL string, options Options, Method string) {

	options.URL = URL
	options.Method = Method
	//TODO add timestamp to request
	opt := CyclePLSRequest{"Queued Request", options}
	response := processRequest(opt)
	client.ReqChan <- response
}

// Do creates a single request
func (client *CyclePLS) Do(URL string, options Options, Method string) (response Response, err error) {
	options.URL = URL
	options.Method = Method

	if client.KeepCookies {
		options.Cookies = append(options.Cookies, client.Cookies...)

		cook := ""
		for _, c := range options.Cookies {
			cook += fmt.Sprintf("%s=%s; ", c.Name, c.Value)
		}

		options.Headers["cookie"] = cook
	}

	opt := CyclePLSRequest{"CyclePLSRequest", options}
	...
	if client.KeepCookies {
		...
		for _, c := range strings.Split(response.Headers["Set-Cookie"], "/,/") {
			Cook := strings.Split(c, "=")
			...
			CookVal := Cook[1]
			if strings.Contains(CookVal, ";") {
				CookVal = strings.Split(CookVal, ";")[0]
			}
...
// Init starts the worker pool or returns a empty cyclepls struct
func Init(workers ...bool) CyclePLS {
